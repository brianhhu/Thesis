...
% formatting required by JNeuro
\usepackage{lineno} % for numbering lines (required by JNeuro)
%\usepackage[nomarkers,figuresonly,fighead]{endfloat} % to place all figures at end 

%% redefine caption function to only show in list, not below actual figure
%\let\OrigCaption\caption
%\renewcommand{\caption}[2][X]{\OrigCaption[#2]{}} % remove caption text from figure
%\AtBeginDelayedFloats{\linespread{1.5}}
...
%%% double space for JNeuro
%\renewcommand{\baselinestretch}{2} % reenable before submitting
%%% line numbers for JNeuro
%\linenumbers % reenable before submitting
...
\ \\
\ \\
{\bf Number of pages:}  % with double space
\\
\ \\
...
% Add Rudiger's grant info here?
We would like to thank Jonathan Williford for helpful discussions on his dataset.
\clearpage

\renewcommand{\thefootnote}{\normalsize \arabic{footnote}} 	

% JNeuro doesn't use keywords, so commented out
%%\ \\[-2mm]
%{\bf Keywords:} border-ownership; recurrent processing; natural scenes; feedback, grouping; perceptual organization
%\clearpage

%\thispagestyle{empty}
%\markboth{}{NC instructions}

% Get across recent results on BOS in natural images, lack of models that can explain these results, and the recurrent structure of our model. Note comparison with Jonathan's data (experimental aspect) and also benchmarking with other computer vision algorithms (engineering aspect). Our model can also provide insight into how the brain solves this problem by providing a model which can be tested on new stimuli
...
% bh is the task referenced here clear?
...
%
% expand on what exactly this result was, close in correlation, Pearson's, statistical testing, etc.?
%
...
%bh required for JNeuro submission (currently 110 words)
\begin{center} {\bf Significance Statement} \end{center} 
% main idea is that our model is one of the *only* models that can explain border ownership coding in natural images, and is also consistent with experimental results (e.g. timing). The feedforward Sakai model is not completely consistent with the physiology. Our model is based on first principles and does not require training, yet does favorably compared to other more sophisticated models that require many more parameters
Figure-ground organization is the process of segmenting an image into regions corresponding to objects. Border-ownership cells are well-suited for this task, as they encode where an object is located relative to an edge in their receptive fields. Here, we propose a model that can explain border-ownership coding in natural scenes. Our model proposes that recurrent connections allow for integration of local and global information about objects, resulting in fast scene segmentation.

\clearpage

% Review literature about border ownership, more recent results on natural scenes, previous computational models/theories of border ownership
\section{Introduction}
%bh Final word count: 
%

% Paper layout outline...feel free to add/modify
\comment{
Intro:

Segmenting image into regions, object boundaries between regions and deciding which objects these belong to
This becomes hard in the presence of clutter, illumination changes, etc.
Importance for visual understanding, prerequisite for object recognition
Understanding neural basis and circuits that give rise to this...

Finding of BOS in V2 neurons (mainly), and move to functional definition of BOS
Shift to ideas about models of how BOS is generated- lateral (Zhaoping), feedforward (Super, Sakai), feedback (Craft)

Figures/Results:

Show example of BOS stimuli (classical standard square), time course here?
Show model components (grouping cell RF)
Show time course of BOS, final outputs (image, edge, BOS, grouping)
	- comment how our model does not require object recognition, short latency
Show scene points with highest/lowest model BOS (analogous cell?)
	- is this better, or show with highest overall FG accuracy
Compare with example cells from Jonathan's paper?
Histogram/scatterplot of BOS for model/cell
Think about how to lesion several grouping layers (local, 3, 5, 7, 9 layers might affect model results...)

Discuss:

Other models
Results on natural scenes
Diversity of cells? (cell-cell comparisons)
Grouping cells- segmentation, saliency, medial axis, etc.
...
% Are the above the most representative citations?
...
% Reference Figure 1/2 here?
%For example, an oriented edge can either belong to an object located to the upper right or to the lower left of the RF (Figure~\ref{Fig:experiments}A). A border-ownership cell will respond differently in these two cases, firing at a higher rate when the object is located on its ``preferred'' side, even though the local stimulus within its RF is identical for both cases.
...
%bh Make more clear distinctions between different types of models
% Need to rework the following two sections since it is copy-pasta from previous paper
...
\hfill
\begin{center}
\includegraphics[width=\textwidth]{figs/exp_data.eps}
\end{center}
...
%
%comment here on contrast or not needed?
%
...
% Mention only one contrast case is shown (results were averaged over both contrasts)
% B) is over all tested cells and scene points
\label{Fig:experiments}
\end{figure}

% Not sure if we should mention this earlier in the intro, or if here is appropriate
%Only more recently has border-ownership coding also been tested on natural stimuli, such as faces \citep{Hesse_Tsao16} and complex natural scenes \citep{Williford_vonderHeydt16}.
...
%
% describe more specifically here when we agree on the analyses
%
...
% Better summary/segue needed here? Kind of abruptly jumps into methods
...
% Mention that BOS cells are omitted in this figure
% Mention only a couple of scales are shown
...
%Consider breaking this up into additional sections?
% In general, probably have to be more clear about real cells vs. model cells, since we propose a hypothetical grouping network. We should also keep the naming convention consistent throughout
...
%In Section 3.2 the grouping mecha-
%nism is extended to operate across multiple feature channels and
%to incorporate competition between proto-objects of similar size
%and feature composition.
%Objects in a visual scene can occlude each other partially. Our
%convention is that in the occlusion zone, we always refer to the object that is closest to the observer as the ‘‘figure,’’ and that behind it
%as the ‘‘background.’’
...
%, $\beta(x,y)$,
 in steps of $\sqrt2$
 % We reference Fig. 3 before Fig. 2- maybe we should flip the figure order?
...
%This model is responsible for estimating the location and spatial
%scale of proto-objects within the input image. 
%grayscale images
%
% Decide if we need to use color for our model
%We first convert the color input image into a grayscale image, as our model
%only operates on luminance information.
%
...
% Describe Simple cell to BOS input
Next, for a given angle~$\theta$, each 
%simple
$\mathcal{S}$
cell
% $\mathcal{S}^k_{\theta}(x,y)$
 feeds into an opposing pair of border-ownership ($\mathcal{B}$) cells.
% , $\mathcal{B}^k_{\theta}$ and $\mathcal{B}^k_{\theta + \pi}$.
As a result, 
...
%We drop the subscripts $L$ and $D$ for notational convenience-- the following operations are applied to both contrast poliarities, as well as to the different color channels.
...
%border-ownership
$\mathcal{B}$
cells with the same preferred orientation but opposing side-of-figure preferences. % Define equations below?
...
%Again, here we drop the $L$ and $D$ subscripts in the grouping cell terms for notational convenience, as the same grouping operations are used for both light and dark objects.
...
\caption{Structure of the recurrent neural network. Each circle stands for a
  ...
  % Think about edge vs. Simple cells, is there a better nomenclature to use?
\label{Fig:model}
\end{figure}
...
% remove delta function, unneeded
% \delta\left(\mathcal{B}^k_\theta(x,y),\hat{\mathcal{B}}\right)\\
% remove w_b parameter
%                    &\times
...
\end{equation}
% provide more intuition why there is a subtractive term here- this is inhibition
...
%$\mathcal{B}$ cells can
%code borders for light objects on dark backgrounds or dark objects on
%light backgrounds. This is achieved by  computing $\mathcal{B}$ cell activity
%independently for each contrast case and then summing it to give
%a border ownership response independent of figure-ground contrast
%polarity. This mimics the behavior of the border ownership cell shown
%in figure~\ref{fig:BOC}.
%en bad link. There are more of them in this file...
...
\end{equation}
%en There is a potential for confusion since B_L can mean both left
%(vs. right) BOS cell as well as light (vs. dark) BOS cell.
%Alex I corrected the figure so L and D only stand for Light and Dark
...
%bh The above could probably be explained a lot better- it is a form of multiplicative feedback, where the activity of BOS cells are either pushed to 0 or up to 2 times their initial activity. When added together, the two activities sum to twice the initial activity, so the total BOS activity is always conserved.
%
...
% maybe need to edit this above section, direct carry-over from Alex's paper draft
...
% which is also
%used for determining the local border orientation, see
%eqs~\ref{eq:border-orientation1} and~\ref{eq:border-orientation2}.
...
% Mention normalization of BOS response between -1 and 1 somewhere here...

% Comment here on color in the model, which is treated similarly to the luminance channel
...
%, but is consistent with psychophysical
%
%I remember Rudiger mentioning some references? 
%
%results which show that grouping is largely based on luminance information~\citep{}.
...
%To compare our model with experimental results, we used a publicly available dataset of border-ownership responses recorded during viewing of squares and natural scenes~\citep{Jonathan_data}.
%% Edit this accordingly...
%We examined two aspects of the dataset. First, we looked at population responses over a variety of scenes. We examined border ownership responses to scenes that were viewed by different cells. We only used cells that had significant responses on the standard border-ownership square test, and also scenes that had been viewed by at least 10 different cells. Second, in the dataset, one cell (Cell 2 in the paper) was tested on almost 200 different scene points, and we applied our model to these same scene points. Our results show sorted border ownership responses according to scene points.
% Evaluate on BSDS-500 instead of BSDS-300? There are 200 new test images, and this is also a more recent dataset released after the original BSDS-300 (also our model has not been overfit on these images, so it is better...)
...
\end{center}
\caption{
%Need to recreate this figure, maybe with new images (column titles, color wheel representation, mean accuracy listed?, etc). 
...
% Should we also show negative examples here of where our model fails?
...
% maybe combine the two time courses on a single plot for better comparison
\label{Fig:results_time}
% Better to show results on artificial stimuli? C-shapes, occluding squares, etc.
\end{figure}
...
% bh Should future work be an additional section?
%- Think about saliency, segmentation using groupingn cell activity
%- Think about experimental predictions?

\clearpage

\bibliographystyle{apalike}
%\bibliographystyle{plain}
\bibliography{niebase,niebase_supp} %,niebase_supp_new}
%\bibliography{/home/niebur/biblio/niebase}%,niebase_supp_new}
%\bibliography{niebase_supp_new}
